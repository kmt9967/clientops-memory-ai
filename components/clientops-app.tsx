...
export function ClientOpsApp() {
  ...
  function sendMessage() {
    ...
    if (/what did we decide|new roofing leads/i.test(text)) {
      ...
      setMemories((current) => current.map((m) => m.id === "m-002" ? { ...m, status: "superseded" as const } : m).concat(extracted));
    ...
    } else if (/doesn['’]?t pick up|unanswered|voicemail/i.test(text)) {
